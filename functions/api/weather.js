...
// Weighted combination of 6 sub-scores, calibrated for Swiss alpine skiing
...
function flatLightScore(dni, diffuse, isDay) {
  if (!isDay) return 0;
  // Near-zero direct sun during daytime = strong flat light
  if (dni != null && dni < 10) return 9;
  if (dni == null || diffuse == null) return 0;
  const total = diffuse + dni + 0.1; // avoid /0
...
function computeHourlyJBI(params) {
  ...
  // Synergy bonus: fresh snow albedo + full cloud + poor visibility = amplified whiteout
  let synergy = 0;
...
export async function onRequestGet(context) {
  ...
  stationCoords.forEach((station, idx) => {
    ...
    // Build cumulative snowfall (last 24h rolling) for albedo proxy
    const cumulSnow24 = [];
    ...
    // Compute JBI per hour, aggregate per day
    const jbiByDate = {};  // { "2026-02-17": { sum, count, peak, hours } }
    hTime.forEach((time, hi) => {
      const dateKey = time.slice(0, 10);
      const isDay = hIsDay[hi] === 1;
      const hour = parseInt(time.slice(11, 13), 10);

      // Ski-relevant hours only (9-16)
    ...
    const forecast = r.daily.time.map((date, di) => {
      ...
      return {
        ...
        // New JBI fields (replaces old jourBlancHours)
